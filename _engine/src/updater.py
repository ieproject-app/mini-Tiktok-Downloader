...
- Self-update: jalankan installer resmi secara detached (background), lalu
  aplikasi keluar; user menjalankan `minitk` lagi setelah 1-2 menit.
"""
...
GITHUB_REPO = "ieproject-app/mini-Tiktok-Downloader"
...
def _parse_ver(version):
    """'v1.2.0' -> (1, 2, 0). Bagian non-numerik diabaikan."""
    parts = []
...
def spawn_installer_update():
    ...
    updater_ps1.write_text(
        "$env:MINITK_SKIP_LAUNCH = '1'\n"
        f"$src = '{INSTALLER_URL}'\n"
        "$tmp = Join-Path $env:TEMP 'minitk-install.ps1'\n"
